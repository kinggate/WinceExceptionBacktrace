...
//
{
	...
	while (dwResult == NO_ERROR)
	{
		...
		if (pchNewline)
		{
			...
			//ASSERT(bytesToCopy < pFile->cbInputBuffer); //lkb

			// Remove the bytes we will copy and the '\n' from the input buffer
			pFile->dwInputBufferOffset += bytesToCopy + 1;
			pFile->cbInputBuffer -= bytesToCopy + 1;

			//ASSERT(pFile->dwInputBufferOffset <= TEXT_FILE_BUFFER_SIZE); //lkb
			//ASSERT(pFile->cbInputBuffer < TEXT_FILE_BUFFER_SIZE); //lkb
...
//
{
	...
	{
#if 0 //lkb del		
		wcsncpy(pNewMapFile->wszModuleName, wszModuleName, MAX_PATH);

		//
		// Build the mapfile name, e.g. L"\Release\coredll.map"
		// 
		//StringCbPrintf(wszMapFileName, sizeof(wszMapFileName), L"\\Release\\%s", wszModuleName);

		_swprintf_l(wszMapFileName, sizeof(wszMapFileName), L"\\Release\\%s", wszModuleName); //lkb
...
#endif
		ptfMapFile = TextFileOpen(wszModuleName); //lkb modify
		if (NULL == ptfMapFile)
		{
			dwResult = GetLastError();
		}
